...
def get_experiment_accessions(accession_set, output_lists, rep_fields):
    ...
    replicates_by_exp = {}
    response = search('FunctionalCharacterizationExperiment', list(accession_set))
    for exp in response:
...
def build_experiments(output_lists, fields_dict, fields_conversion, objs_by_type, objs_by_id):
    ...
    experiment_df = get_report_tsv_from_fields('FunctionalCharacterizationExperiment', experiments, experiment_fields_list)
...
def search(object_type, accession):
    ...
    max_url_length = 8192
    if object_type != "FunctionalCharacterizationExperiment":
    ...
    if object_type == 'File':
        ...
        experiment_id = '/functional-characterization-experiments/{}/'.format(accession)
        ...
        res = response.json()
        return res['@graph']
    elif object_type in ('Experiment', 'FunctionalCharacterizationExperiment', 'Biosample'):
...
def get_report_tsv_from_fields(object_type, accession, fields_list):
    ...
    if isinstance(accession, list):
        ...
        for chunk in chunked:
            ...
            URL = SERVER + url_part_1 + url_part_2
            response = requests.get(URL, auth=(AUTHID, AUTHPW))
            res_df = pd.read_csv(io.StringIO(response.text), sep = '\t', header=1)
            dfs.append(res_df)
...
keypair = getKeyPair('keypairs.json', 'production')
