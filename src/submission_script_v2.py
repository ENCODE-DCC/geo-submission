...
def main():
    ...
    donor_fields_direct_conversion = {'Binomial name': 'organism', # organsim.scientific_name
                               'Title': 'lab', # lab.title
                               'URL': 'url',
    ...
    file_fields_conversion = {'Alternate accessions': 'alternate_accessions', 'File status': 'status', 
                              'Paired End Identifier': 'paired_end', 'Derived from': 'derived_from', 
                              'File pairing': 'paired_with', 'Mapping assembly': 'assembly', 
                              'Genome annotation': 'genome_annotation', 'Accession': 'accession', 
                              'MD5sum': 'md5sum', 'Data type': 'output_type', 'File Format': 'file_format', 
                              ...
                              'File size': 'file_size', 'Run type for sequencing files': 'run_type', 
                              'Data category': 'output_category', 
...
def build_donors(output_lists, report_fields, fields_conversion, objs_by_type, objs_by_id):
    ...
    for donor_type, donors in output_lists['donors'].items():
        ...
        for i in donor_df.index:
            ...
            if donor_df.loc[i, 'Binomial name'] not in desired_species:
                continue
            else:
